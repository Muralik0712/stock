...
def getPredictions():
    if (request.method == "POST"):
        ...
        import pandas as pd
        fn='C:\\Users\\veena\\\OneDrive\\Desktop\\stockfinal\\stockproduct\\Project\\backend\\datasets\\' + modelName+'.csv'
        df=pd.read_csv(fn)
        dffd=df
        plotcandle.cplot(dffd)
...
def getManualPrediction():
    if (request.method == "POST"):
        ...
        session['prediction']['manualPrediction'] = str('Predicted Price - '+str(prediction)+' Open Price - '+str(openValue)+' High Price - '+str(highValue)+' Low Price - '+str(lowValue)+' Volume - '+str(volumeValue))
        #session['alldata']['manualPrediction']=str('Open - '+openValue +' High - '+highValue+' Low - '+lowValue+' Close - '+volumeValue)
